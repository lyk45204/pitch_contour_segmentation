...
labels = {-1, 0, 1, -2}
[features, labeled_quanta] = extract_quantums_and_features(trainData, stateName, labels); % include nosie state segment
...
pitchContourSegments = ExtractPitchContours(trainData,labeled_quanta);

%% train the model using the BestbandwidthMultipliers
stateName = {'transitory', 'steady', 'modulation', 'noise'};
statelabels = {-1, 0, 1, -2};
BestbandwidthMultipliers = 6;
trainingSet = pitchContourSegments;
[transitionMatrix, observationLikelihoods, bandwidth] = trainHMM_KDE(trainingSet, stateName, statelabels, BestbandwidthMultipliers);
...
performance_frame_mean = validateHMM(transitionMatrix, observationLikelihoods, testSet);
